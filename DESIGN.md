...
**Контекст.** I-0010 проектирует кликабельные ref-ссылки в COIL-H, но не фиксирует, как рендерить три краевых случая:
...
**Контекст.** Визуальная проверка обнаружила, что ссылки `?promise` в `WAIT ON ?security_review` рендерятся как unresolved, хотя `?security_review` — это handle для уже задекларированного `Op.Think $security_review`. Индекс по правилам I-0011 пишет только `$<name>`, не `?<name>` — поэтому лукап `?security_review` промахивается.
...
## I-0015 — Editor highlighting: Monaco tokens из runtime tokenizer (отказ от monarch)
...
| **Связан с** | D-0050 (heredoc-форма), R-0021/R-0023 (lexer токены) |

**Контекст.** Подсветка COIL в Monaco-редакторе сейчас построена на декларативной monarch-грамматике (`coil-ide/src/coil/monarch.ts`) — параллельной runtime-парсеру. Эта грамматика покрывает стандартный шаблон `<< ... >>` и набор ключевых слов из диалекта, но **не моделирует heredoc** (`<<TAG ... TAG`, `<<'TAG' ... TAG` — D-0050): открывающий и закрывающий теги не различаются визуально, незакрытый heredoc не диагностируется, содержимое тела может «протекать» в подсветку соседних операторов.

Runtime-tokenizer (`coil-runtime/src/lexer/tokenizer.ts`) полностью знает все три формы шаблонов: эмитит типизированные токены `TemplateOpen` / `TemplateClose` / `HeredocOpen` (с маркером и raw-флагом) / `HeredocClose`, и ловит ошибки `unterminated-template` / `unterminated-heredoc` / `heredoc-expected-marker` / `heredoc-unclosed-quote`. Он **уже экспортируется публично** через `coil-runtime/browser` (`tokenize`, типы токенов).

Это создаёт два параллельных источника лексической грамматики: runtime — глубокий и точный, monarch — поверхностный и stateless. Heredoc — первая фича, на которой дрейф стал видимым; следующие будут (например, типизированные comparison-операторы R-0023, контекстно-зависимые ключевые слова диалектов). **Гипотеза фаундера** (либо IDE неправильно использует runtime, либо у runtime плохая архитектура) переформулируется точнее: **runtime-API в порядке, IDE его не использует для подсветки** — потому что Monaco исторически предлагает monarch как «лёгкий путь» старта. Этот лёгкий путь упирается в потолок выразительности.

**Решение.**

Перевести подсветку COIL в Monaco на программный `monaco.languages.setTokensProvider`, который использует `tokenize` из `coil-runtime` как единственный источник истины. Monarch-грамматика удаляется.

Архитектура adapter'а:

1. `setTokensProvider(languageId, provider)` — получает наш `provider` с методами `getInitialState()` и `tokenize(line, state)`.
2. Внутри adapter'а кэшируем результат `tokenize(model.getValue())` по версии модели Monaco. Кэш инвалидируется при изменении документа (Monaco пересоздаст state при `getInitialState`).
3. Каждый вызов `provider.tokenize(line, state)` отдаёт токены текущей строки из кэша, в формате Monaco (`{ tokens: [{ startIndex, scopes }], endState }`).
4. Маппинг токенов runtime → Monaco token classes: `Operator → 'keyword.operator'`, `Modifier → 'keyword.modifier'`, `TemplateOpen/Close, HeredocOpen/Close → 'delimiter.template'`, `TextFragment внутри template → 'string.template'`, `ValueRef → 'variable'`, `ParticipantRef → 'entity.participant'`, и т.д. — таблица в коде adapter'а.
5. Heredoc-маркеры: `HeredocOpen.marker` подсвечивается отдельным цветом-подклассом (`'delimiter.heredoc-marker'`), парность с `HeredocClose` достигается визуально через одинаковую подсветку маркера и общую цветовую группу.
6. Незакрытый heredoc / template: `LexerError` с кодом `unterminated-*` уже летит из `tokenize`. Adapter ловит, **не пропадает** — отдаёт частичные токены до точки ошибки и одновременно публикует диагностику через `monaco.editor.setModelMarkers`. Это интегрируется с существующим pipeline валидации (там же лежат `ValidationPanel`-диагностики).

**Почему.**

- **Один источник истины.** Любая лексическая фича языка автоматически попадает в подсветку — больше нет дрейфа. Heredoc, comparison-операторы, контекстные ключевые слова диалектов — всё «бесплатно».
- **Реальная семантика heredoc.** Маркер из `<<TAG` сопоставляется с `TAG` на закрывающей строке точно (а не «первый встречный идентификатор», как пришлось бы делать в monarch).
- **Диагностики из лексера попадают в редактор.** `unterminated-heredoc` теперь — реальная ошибка с подсветкой, а не невидимый сбой.
- **Архитектурно правильнее.** monarch-грамматика — это «лёгкий старт» Monaco; для языка с собственным полноценным tokenizer'ом параллельная грамматика — техдолг. Monaco поддерживает программные `tokensProvider` именно для таких случаев.
- **Не требует изменений в runtime.** `tokenize` уже публичный, токены типизированы. Adapter живёт целиком в `coil-ide`.

**Цена.**

- Adapter в `coil-ide` — новый код (~150–250 строк): инициализация, маппинг, кэш по версии модели, интеграция с Monaco markers.
- Удаление `monarch.ts` (123 строки) и связанных тестов (если есть).
- Производительность: full re-tokenize всего документа на каждое изменение. Для типичных файлов COIL (десятки–сотни строк) — пренебрежимо. Для файлов в тысячи строк — потенциальное узкое место; если станет проблемой — отдельная задача (инкрементальный tokenizer, throttle).
- Маппинг token-class → CSS-цвет: нужно проверить, что текущая тема Monaco покрывает все классы; добавить недостающие в `themes.ts`.
- Тесты: smoke-тест на adapter'е (создать model, проверить токены для типичного и сломанного heredoc-файла). Обязательно для регрессий.

**Альтернативы (отвергнуты).**

- **A. Расширить monarch для heredoc.** monarch-state-machine не запоминает значение marker'а — невозможно сопоставить `<<TAG` с конкретным `TAG` на закрытии. Можно сделать «первый встречный идентификатор закрывает heredoc» — даст ложные срабатывания, не покрывает спеку. Латание, которое продлит дрейф.
- **C. Гибрид: monarch + runtime decorations поверх.** Две системы вместо одной — концептуальный дубль ответственности. Дрейф продолжится в новых местах.

**Будущее.** Если появится потребность в подсветке вне Monaco (VSCode-extension через TextMate, LSP, server-side syntax highlighter) — тогда оценить, стоит ли **генерировать** декларативные грамматики из runtime tokenizer или поддерживать их вручную. На горизонте 6 месяцев таких потребителей не видно — преждевременно проектировать.
